...
public:
  	...
  	void Insert(const char * str,int pri)
  	{
        ...
        uint tmp=bobhash[counter_num]->run(str,strlen(str));
        int from=((tmp>>24)&0xFF)%counter_num;
        index[counter_num]=(tmp&0xFFFFFF)%w;
        if(counter[from][index[counter_num]]!=MAX_CNT)
            counter[from][index[counter_num]]++;

        from++;
        if(from>=counter_num)from-=counter_num;
        //int to=from+LOW_HASH_NUM+pri-1;

        for(int i=1;i<LOW_HASH_NUM+pri;i++)
        {
            //if(from!=0)
    ...
    int Query(const char *str,int pri)
    {
        ...
        temp=counter[from][index[counter_num]];
